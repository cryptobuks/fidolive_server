<template>
    <div class="equipment-list">
        ...
        <el-table :data="tableData" border :empty-text="$store.state.langData.cont.msg.data.d0001" style="width: 100%">
            ...
            <el-table-column :label="$store.state.langData.cont.pageFn.table['Network Info']" v-if="$store.state.gobalData.me.roleCode === 1">
                <template slot-scope="scope">
                    <el-link v-if="scope.row.status" type="success" @click.native.prevent="handlePi(scope.$index, scope.row)">{{ scope.row.ipv4 }}</el-link>
                    <p style="margin: 0;">{{ scope.row.mac }}</p>
                </template>
            </el-table-column>
            <el-table-column label="Zero Tier" v-if="$store.state.gobalData.me.roleCode === 1">
                <template slot-scope="scope">
                    <p style="margin: 0;">{{ scope.row.z_ipv4 }}</p>
                    <p style="margin: 0;">{{ scope.row.z_mac }}</p>
                </template>
            </el-table-column>
            <el-table-column :label="$store.state.langData.cont.pageFn.table.version" v-if="$store.state.gobalData.me.roleCode === 1">
                <template slot-scope="scope">
                    <p style="margin: 0;">{{ scope.row.version }}</p>
                </template>
            </el-table-column>
            <el-table-column :label="$store.state.langData.cont.pageFn.table.Operating" width="100">
                <template slot-scope="scope">
                    <p style="margin: 0;">
                        <el-button @click.native.prevent="handleEdit(scope.$index, scope.row)" type="text" style="line-height: 0;">
                            {{ $store.state.langData.cont.pageFn.table.Edit }}
                        </el-button>
                    </p>
                    <p style="margin: 0;">
                        <el-button @click.native.prevent="handleDelete(scope.$index, scope.row)" type="text" style="line-height: 0;">
